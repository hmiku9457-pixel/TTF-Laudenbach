...
# Generierte Entwicklungs- und Testartefakte sind keine Bestandteile der Website.
IGNORE_PARTS = {
    ".git",
    "__pycache__",
    "node_modules",
...
def check_html(errors: list[str], warnings: list[str]) -> None:
    ...
    for path in iter_files("*.html"):
        ...
        for element in soup.find_all(True):
            for attribute in element.attrs:
                if INLINE_EVENT_RE.match(attribute):
                    errors.append(
                        f"{rel}: Inline-Event-Handler nicht erlaubt: {attribute}"
                    )
            ...
            if reference:
                ...
                if target and not target.exists():
                    errors.append(
                        f"{rel}: {element.name}-Referenz fehlt: {reference}"
                    )
        ...
        if len(skip_links) != 1:
            errors.append(
                f"{rel}: Skip-Link zum Hauptinhalt fehlt oder ist doppelt"
            )
        ...
        if styles.count("/assets/css/site.bundle.css") != 1:
            errors.append(
                f"{rel}: erwartet genau eine Referenz auf "
                "/assets/css/site.bundle.css"
            )

        if any(
            value and (
                value.endswith("/style.css")
                or value.endswith("/main.css")
            )
            for value in styles
        ):
        ...
        if scripts.count("/assets/js/main.js") != 1:
            errors.append(
                f"{rel}: erwartet genau eine Referenz auf /assets/js/main.js"
            )
        ...
        if any(
            value and value.endswith("/script.js")
        ...
        for index, table in enumerate(soup.find_all("table"), start=1):
            if not table.find("caption"):
                errors.append(
                    f"{rel}: Tabelle {index} hat keine Beschriftung (<caption>)"
                )
            ...
            if thead:
                for header in thead.find_all("th"):
                    if header.get("scope") != "col":
                        errors.append(
                            f'{rel}: Tabellenkopf ohne scope="col"'
                        )
...
def check_sitemap(errors: list[str]) -> None:
    ...
    for location in locations:
        ...
        if not target.exists():
            errors.append(
                f"sitemap.xml verweist auf fehlende Seite: {location}"
            )
...
def check_bundle(errors: list[str]) -> None:
    result = subprocess.run(
        [
            sys.executable,
            "tools/review-upgrade/build_css.py",
            "--check",
        ],
        cwd=ROOT,
        check=False,
    ...
    if result.returncode:
        errors.append(
            ...
            or "CSS-Bundle ist nicht aktuell"
        )
...
def main() -> int:
    ...
    print(
        f"Qualitätsprüfung erfolgreich ({len(warnings)} Warnungen)."
    )
    return 0
